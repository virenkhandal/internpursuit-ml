...
def match(students, employers, company_name, count, api=False):
    ...
    for i in range(len(students.index)):
        ...
        if api == True:
            ...
            filtered = round1_filter(employers, curr)
            text = "Employers remaining after first round: " + str(len(filtered))
            print(colored(text, "magenta"))
            # filtered = students
            # create dataframe with filtered students and i-th employer
            appended = filtered.append(curr)
            ...
            s_optimal_matchings = match_skills(s_clustered)
            db_optimal_matchings = match_skills(db_clustered)
            text = "Employers remaining after second round: " + str(len(s_optimal_matchings))
            print(colored(text, "magenta"))
...
if __name__ == "__main__":
    ...
    match('updated_students.csv', 'updated_employers.csv', "", 0)